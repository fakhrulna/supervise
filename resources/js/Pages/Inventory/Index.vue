<template>
    <MainLayout>
        <!-- Table Section -->
        <div class="max-w-[85rem] px-4 py-10 sm:px-4 lg:px-5 lg:py-5 mx-auto">
            <div class="flex flex-col">
                <span class="align-top top-0 pb-4 text-xl font-semibold">Inventory List</span>
            </div>
            <!-- Card -->
            <div class="flex flex-col">
                ...
                <div class="p-1.5 min-w-full inline-block align-middle">
                    ...
                    <div class="px-6 py-4 grid gap-3 md:flex md:justify-between md:items-center border-t border-gray-200">
                        ...
                        <select class="py-2 px-3 pe-9 block border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500">
                            <option>1</option>
                            <option>2</option>
                            <option>3</option>
                            <option>4</option>
        ...
        <!-- End Table Section -->
         <!-- Modal Component -->
      <!-- <Modal :isOpen="isModalOpen" @close="closeModal" @save="handleSave" /> -->
      <!-- <AddForm class="max-w-xl" :isOpen="isModalOpen" @close="closeModal" @save="handleSave" /> -->
      <AddForm :isOpen="isModalOpen" @close="closeModal" @save="handleSave" />
    </MainLayout>>
</template>
...
export default {
  ...
  methods: {
    ...
    handleSave(newData) {
      ...
      this.isModalOpen = false;
      newData.post(route('inventory.store'), {
        onSuccess: () => newData.reset(),
        });
    },
    // handleSave(newData) {
    //     // Send data to the backend
    //     this.$inertia.post(route('inventory.store'), newData, {
    //         onSuccess: () => {
    //             this.closeModal();
  ...
  mounted() {
        ...
        // this.$inertia.get(route('inventory.index'), {
        //     onSuccess: (response) => {
        //         this.inventory = response.props.inventory;
        //     },
        // });
    },
};
